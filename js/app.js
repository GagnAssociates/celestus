...
//Chargement des scripts de l'application
        if ($(location).attr('href').indexOf('dev') >= 0 || $(location).attr('href').indexOf('localhost') >= 0) {
            requirejs(['calcs/Global'], function () { requirejs(['calcs/formules'], function () { requirejs(['calcs/Interface']);});});
    $('head').append('<link rel="stylesheet" href="css/style.css" />');
}
else {
            requirejs(['calcs/Global.min'], function () { requirejs(['calcs/formules.min'], function () { requirejs(['calcs/Interface.min']); }); });
